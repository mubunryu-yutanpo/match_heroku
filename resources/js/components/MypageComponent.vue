    ...
    <div class=""><!-- 全体のwrap -->
        ...
        <section class="">
            ...
            <div class="p-list">
                ...
                <div class="p-list__container" v-for="post in postList" :key="post.id">
                    <p class="c-type">{{ post.type.name }}</p>
                    <a :href="'/project/' + post.id + '/detail'" class="c-link p-list__link">{{ post.title }}</a>
                    <p class="p-list__content">{{ post.content }}</p>
                </div>
                <a :href="'/postList/'+ this.user.id" class="c-link p-list__link">全件表示する</a>
            </div>

            <!-- 応募案件一覧 -->
            <div class="p-list">
                <h3 class="c-title p-list__title">応募した案件一覧</h3>
                <div class="p-list__container" v-for="apply in applyList" :key="apply.id">
                    <p class="c-type">{{ apply.project.type.name }}</p>
                    <a :href="'/project/' + apply.project.id + '/detail'" class="c-link p-list__link">{{ apply.title }}</a>
                    <p class="p-list__content">{{ apply.content }}</p>
                </div>
                <a :href="'/applyList/'+ this.user.id" class="c-link p-list__link">全件表示する</a>
            </div>

            <!-- パブリックM一覧 -->
            <div class="p-list">
                <h3 class="c-title p-list__title">パブリックメッセージ一覧</h3>
                <div class="p-list__container" v-for="pubMessage in publicMessageList" :key="pubMessage.id">
                    <p class="c-type">{{ pubMessage.project.type.name }}</p>
                    <a :href="'/project/' + pubMessage.project.id + '/detail'" class="c-link p-list__link">{{ pubMessage.project.title }}</a>
                    <p class="p-list__content">{{ pubMessage.comment }}</p>
                </div>
                <a :href="'/publicMessageList/'+ this.user.id" class="c-link p-list__link">全件表示する</a>
            </div>

            <!-- DM一覧 -->
            <div class="p-list">
                <h3 class="c-title p-list__title">ダイレクトメッセージ一覧</h3>
                <div class="p-list__container" v-for="dm in directMessageList" :key="dm.id">
